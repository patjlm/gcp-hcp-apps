        ...
        if isinstance(sequence_data, list):
            ...
            for item in sequence_data:
                if isinstance(item, dict) and "name" in item:
                    ...
                    child_keys = [
                        k for k in item.keys() if k not in ["name", "promotion"]
                    ]
                    if child_keys:
                        # Has children, recurse
                        for child_key in child_keys:
                            targets.extend(
                                traverse_dimensions(item[child_key], new_path)
                            )
                    else:
...
def merge_application_values(
    ...
    if defaults_file.exists():
        ...
        if "defaults" in defaults:
            merged = deep_merge(
                merged, {"applications": {app_name: defaults["defaults"]}}
            )
    ...
    if not app_values_file.exists():
        raise FileNotFoundError(
            f"Mandatory application values file not found: {app_values_file}"
        )
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if chart_file.exists():
            ...
            chart_data["description"] = (
                f"ArgoCD applications for {cluster_type} in {target.path}"
            )
        ...
        try:
            ...
            filtered_lines = [
                line for line in lines if not line.startswith("# Source:")
            ]
            clean_output = "\n".join(filtered_lines)
